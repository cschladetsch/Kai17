...
Token Tokenizer::scanToken() {
    ...
    switch (c) {
        ...
        case '\t':
        case '\r':
            return Token(Token::Type::Whitespace, std::string(1, c), line_, column_ - 1);
        case '\n':
        ...
        case ',':
        case ';':
            return Token(Token::Type::Punctuator, std::string(1, c), line_, column_ - 1);
        case '"':
            return scanString();
        default:
            if (isDigit(c)) {
...
Token Tokenizer::scanNumber() {
    ...
    Token::Type type = isFloat ? Token::Type::FloatingPoint : Token::Type::Integer;
...
Token Tokenizer::scanString() {
    size_t start = position_;
    size_t startColumn = column_ - 1;  // -1 because we've already consumed the opening quote
